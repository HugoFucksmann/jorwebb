...
const crearMascotaf = async (req, res) => {
  try {
    ...
    const mName = JSON.stringify(mascota.date);

    const fileName = `${mName + "." + extensionArchivo}`;

    const drive = await conectDrive();
    ...
    const result = await uploadFile(drive, fileName, fileData, extensionArchivo);
    ...
    const uid = mascota.usuario;

    const mascotaf = new Mascota({
      ...mascota,
      petPicture: url.webContentLink,
    });

    const mascotaDB = await mascotaf.save();
    ...
    usuarios.map((usuario) => {
      ...
      if (usuario._id !== uid && distancia < 2) {
        messages.push({
          to: usuario.notification,
          sound: "default",
          title: "se perdio una mascota !!",
...
const borrarMascotaf = async (req, res = response) => {
  const mascotaId = req.params.id;
  ...
  try {
    ...
    const [beg, end] = mascotaDB.petPicture.split("id=");
    const [fileId, rest] = end.split("&");
